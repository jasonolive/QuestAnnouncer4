QuestAnnouncer4 = LibStub("AceAddon-3.0"):NewAddon("QuestAnnouncer4", "AceConsole-3.0", "AceComm-3.0", "AceEvent-3.0", "AceSerializer-3.0");
...
function QuestAnnouncer4:OnInitialize()
	...
	self:Print("QuestAnnouncer 4 has successfully loaded.");
end
...
function QuestAnnouncer4:ProcessAnnouncement(typ, msg)
	...
	local color = {};
	if (typ == 0) then
		broadcast = string.format("%s\'s quest progress... %s", UnitName("player"), msg);
		color = {r=1,g=1,b=0};
	elseif (typ == 1) then
		broadcast = string.format("%s has completed %s", UnitName("player"), msg);
		color = {r=0,g=1,b=0};
		UIErrorsFrame:AddMessage(string.format("You have completed %s", msg),color.r,color.g,color.b,1.0,UIERRORS_HOLD_TIME);
	elseif (typ == -1) then
		broadcast = string.format("%s has failed %s", UnitName("player"), msg);
		color = {r=1,g=0,b=0};
		UIErrorsFrame:AddMessage(string.format("You have failed %s", msg),color.r,color.g,color.b,1.0,UIERRORS_HOLD_TIME);
	end
	--if (GetNumPartyMembers()>0) then
		local serializeddata = self:Serialize(broadcast, color);
...
function QuestAnnouncer4:UI_INFO_MESSAGE(frame,msg)
	if (msg ~= gsub(msg,"(.*):%s*([-%d]+)%s*/%s*([-%d]+)%s*$","%1",1)) then
		self:ProcessAnnouncement(0, msg);
	end
end
...
function QuestAnnouncer4:QUEST_LOG_UPDATE()
	...
	while (GetQuestLogTitle(QuestID) ~= nil) do
		...
		if (not isHeader) then
			...
			if (questDescription) then
				questCRC = self:CRC32(questDescription);
				local status = 0
				if (isComplete) then status = isComplete end;
				tmpQuestLog[questCRC] = status;
				-- Check for Quest changes
				if questLog[questCRC] and questLog[questCRC] ~= tmpQuestLog[questCRC] then
					if questLog[questCRC] == 0 and tmpQuestLog[questCRC] == 1 then
						self:ProcessAnnouncement(1, questTitle);
					elseif (questLog[questCRC] == 0 or questLog[questCRC] == 1) and tmpQuestLog[questCRC] == -1 then
						self:ProcessAnnouncement(-1, questTitle);						
					end
				end
			end
	...
	-- Switch the two tables, so that tmpQuestLog becomes questLog
	questLog = tmpQuestLog;
...
function QuestAnnouncer4:CRC32(s)
